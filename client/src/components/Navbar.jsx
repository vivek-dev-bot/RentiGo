...
const Navbar = () => {
  ...
  const changeRole = async() => {
    try {
      ...
      if(data.success){
        ...
        toast.error("Only owner can list cars")
  ...
  return ( 
    ...
    className={`flex item-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4
...
/>
      ...
      <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16
            ...
            items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duraction-300 z-50 
        ...
        <div className='hidden lg:flex items-cenert text-sm gap-2 border border=boderColor px-3 rounded-full max-w-56'>
          <input type="text" className='py-1.5 w-full bg-transparent outline-none placeholer-gray-500' placeholder='Search products' />
          <img src={assets.search_icon} alt="seah" />
        </div>
...
<button className='sm:hidden cursor-ponter' aria-label='Menu' onClick={() => setOpen(!open) }>
