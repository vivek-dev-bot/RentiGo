...
const CarDetails = () => {
  ...
  const { id } = useParams()
  const { cars, axios, pickupDate, setPickupDate, returnDate, currency, setReturnDate } = useAppContext()
  ...
  return car ? (
    <div className='px-6 md:px-16 lg-px-24 xl-px:32 mt-16'>
      <button onClick={() => navigate(-1)} className='flex items-center gap-2 mb-6 text-grey-500 cursor-pointer'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12'>
        ...
        <form onSubmit={handleSumbit} className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
          ...
          <button className='w-full bg-primary hover:bg-primary-dull transi py-3 font-medium text-white rounded-xl cursor-pointer'>Book Now</button>
